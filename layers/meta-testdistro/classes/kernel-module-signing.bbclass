DIGSIG_REST_ENDPOINT = "/sign/modules"

inherit signing_server

PACKAGE_DEPENDS += "${@' '.join([dep.split(':')[0] for dep in d.getVar('DIGSIG_DEPS').split()])}"
PACKAGEBUILDPKGD_append_jetson-tx2-cboot = " sign_modules"

PKGD_SIGNED = "${WORKDIR}/package-signed"

sign_modules() {
    tar -C ${PKGD} -c -z -f ${PKGD_SIGNED}/modules-in.tar.gz ${@d.getVar('nonarch_base_libdir')[1:]}/modules
    digsig_post -F "machine=${MACHINE}" -F "hashalg=sha512" -F "artifact=@${PKGD_SIGNED}/modules-in.tar.gz" --output ${PKGD_SIGNED}/modules-out.tar.gz
    tar -C ${PKGD} -x --no-same-owner -f ${PKGD_SIGNED}/modules-out.tar.gz
}
